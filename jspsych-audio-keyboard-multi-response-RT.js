...
jsPsych.plugins["audio-keyboard-multi-response-RT"] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('audio-keyboard-multi-response-RT', 'stimulus', 'audio');
...
//    display_element.append('');
    ...
    var boop = function() {
      ...
      setTimeoutHandlers.push(setTimeout(function() { end_trial(); }, trial.timing_after_response));
    }
...
//       if (trial.visual_feedback === 'detec') {text_element.type = 'password'};
        if (trial.visual_feedback === 'aster') {
          text_element.className = 'pass';
          text_element.style.fontSize = '38px'};
...
//        text_element.style.fontSize = '38px'; //26px
...
//      }
    var trial_data = {
        ...
        "rtlast": responseTimes[responseTimes.length-1],
...
//        "tt": JSON.stringify(responseTypingTimes),
        ...
        "audioend": audioEndTime,
        "condition": trial.visual_feedback,
        "isbackspace":isbksp,
//        "target": target, // doesn't work when there is a path for target in an audio folder
      };
...
//    var target = trial.stimulus.substring(0,trial.stimulus.length-4); // this doesn't work while there is a path for audio files
    var target = trial.target;
    var bksp = 0;
    var lettercount = 0;
...
//            var lettercount = counter - 2*bksp;
              ...
              lettercount += 1;
//              console.log(info.key);
//              console.log(target[lettercount-1]);
